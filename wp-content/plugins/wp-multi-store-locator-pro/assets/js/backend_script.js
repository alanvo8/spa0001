...
jQuery(document).ready(function (jQuery) {
  ...
  if ( jQuery('.import_js_stores').attr('disabled') != "disabled" ) {
							...
							jQuery.ajax({
								...
								data: 'action=import_js_stores_action&import_js_stores_post=' + JSON.stringify(stores_json_encoded[i]),
									...
									setTimeout(function(){ 		
										...
										jQuery('#current-count').text(del+1);
										del++;
									}, 500);
							  });
							 i++;
	...
	jQuery( ".clear_imported_stores" ).click(function(e) {
		
        jQuery.ajax({
            url: ajax_url,
            data: 'action=clear_imported_stores_action&clear_imported_stores=true',
            type: 'POST',
            success: function (response) {
                if(response=='true'){
                    jQuery('#store_locator_message').html("<p>Stores Deleted.</p>");
                    jQuery('#tolalcount').html("0");
                    jQuery('#current-count').html("0");
                    jQuery('.stores_div').html(" ");

                    stores_json_encoded = [];
                    
                }
                else{
                    jQuery('#store_locator_message').html("<p>Could not delete stores.</p>");
                }
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                jQuery('#store_locator_message').html("<p>Ajax Request Failed.</p>");
            }
        });
