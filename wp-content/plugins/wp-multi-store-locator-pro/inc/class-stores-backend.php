...
if(!class_exists('WPMSL_Stores_Backend')){
	class WPMSL_Stores_Backend{
		...
		public function register_stores_submenu_page() {
		    add_submenu_page('edit.php?post_type=store_locator', __('Settings','store_locator'), __('Settings','store_locator'), 'manage_options', 'store_locator_settings_page', array($this,'store_locator_settings_page_callback'));
		}
		...
		public function store_locator_backend_script() {
		    ...
		    if( $post_type  == 'store_locator' 
		        ...
		        wp_enqueue_script('select2', STORE_LOCATOR_PLUGIN_URL . '/assets/js/select2.js');
		        wp_enqueue_style('select2', STORE_LOCATOR_PLUGIN_URL . '/assets/css/select2.css');
		        wp_enqueue_script('timepicker', STORE_LOCATOR_PLUGIN_URL . 'assets/js/jquery.timepicker.js');
                wp_enqueue_style('timepicker', STORE_LOCATOR_PLUGIN_URL . 'assets/css/jquery.timepicker.css');
		...
		public function sample_admin_notice__error($array) {
		    ...
		    if(empty($nomaps)){
		        $message = sprintf('<p>'.__( 'Thanks for updating to WP Multistore locator 2.0. You can now create multiple map instance with lots of new features. So what are you waiting for? %s', 'store_locator' ).'</p>','<p><a href="'.esc_url(admin_url('post-new.php?post_type=maps')).'" class="wpml maps notice">'.__("Create New Map","store_locator").'</a></p>');
		...
		public function store_locator_custom_dashboard_widget_callback() {
		    ...
		    $transactions = $wpdb->get_results("SELECT ps.post_title as store, count(tr.post_id) as total_count FROM $posts_table ps INNER JOIN store_locator_transactions tr ON tr.post_id=ps.ID WHERE ps.post_type='store_locator' AND ps.post_status='publish' GROUP BY ps.ID ORDER BY total_count DESC LIMIT 3");
		    if($transactions){
		        ?>
		        <table class="store_locator_data_dashboard">
		            <tr>
		                <th><?php esc_html_e("Store Name", 'store_locator'); ?></th>
		                <th><?php esc_html_e("Hits", 'store_locator'); ?></th>
		            </tr>
		        ...
		        <a href="<?php echo admin_url('edit.php?post_type=store_locator&page=statistics_submenu_page'); ?>" ><?php esc_html_e("See more ...","store_locator"); ?></a>
		        <?php
		    }else{
        ...
        public function map_shortcode_matabox_callback($post){
            ...
            <div class="map_shortcode_callback">
                ...
                <?php  
                echo '<p> <input type="text" value="[wp_multi_store_locator_map id='.$post->ID.']" readonly="readonly" name="shortcode"> </p>'; ?>
            </div>
            <?php 
            echo ob_get_clean();
        }
        public function map_layout_matabox_callback($post){
            global $post;
            $map_layouts=get_post_meta($post->ID, 'map_layouts', true);
            ob_start(); ?>
            <div class="map_layout_callback">
                <?php  ?>
                <label title="Select Map layout type" for="store_locator_map_layout"><?php esc_html_e('Map Layout Type', 'store_locator'); ?>:</label>               
                <select name="map_layouts[layout]" id="store_locator_map_layout" >
                    <option  <?php echo (isset($map_layouts['layout']) && $map_layouts['layout'] == 'custom') ?"selected": ""; ?> value="custom">
                        <?php esc_html_e('Custom','store_locator'); ?>
                        </option>
                        <option  <?php echo (isset($map_layouts['layout']) && $map_layouts['layout'] == 'fullscreen') ?"selected": ""; ?> value="fullscreen">
                        <?php esc_html_e('Fullscreen','store_locator'); ?>
                        </option>                    
                </select>
            </td></tr>
            </div>
        ...
        <div class="inside store_locator_map_settings" <?php echo (isset($map_options['global']) &&  $map_options['global']=='yes') ? 'style="display:none;"' : ''; ?>>
            <table class="wpml-table">
            	<tbody>
                <tr><th>
                <label title="<?php esc_html_e('Enable the display of map on the frontend','store_locator'); ?>" for="store_locator_map_enable"><?php esc_html_e("Show map on frontend", 'store_locator'); ?>?</label>
            </th><td>
            ...
            <tr><th>
                <label title="<?php esc_html_e('Select Map Width pixels','store_locator'); ?>" for="store_locator_map_width"><?php esc_html_e('Map Width', 'store_locator'); ?>:</label>
            </th><td>
            ...
            <tr><th>
                <label title="<?php esc_html_e('Select Map Height pixels. Min height 500px','store_locator'); ?>" for="store_locator_map_height"><?php esc_html_e("Map Height ", 'store_locator'); ?>:</label>
            </th><td>
            ...
            <tr><th>
                <label title="<?php esc_html_e('Select Map Type','store_locator'); ?>" for="store_locator_map_type"><?php esc_html_e("Map Type", 'store_locator'); ?>:</label>
            </th><td>
            ...
            <tr><th>
                <label title="<?php esc_html_e('Choose the unit of search km/mile','store_locator'); ?>" for="store_locator_map_unit"><?php esc_html_e("Search Unit", 'store_locator'); ?>:</label>
            </th><td>
                <select name="store_locator_map[unit]" id="store_locator_map_unit">
                    <option <?php echo (isset($map_options['unit']) && $map_options['unit'] == 'km') ?"selected=": ""; ?> value="km">Km</option>
                    <option <?php echo (isset($map_options['unit']) && $map_options['unit'] == 'mile') ?"selected=": ""; ?> value="mile">Mile</option>
                </select>
            </td></tr>
            <tr><th>
                <label for="store_locator_direction"><?php esc_html_e("Direction", 'store_locator'); ?>:</label>
            </th><td>
                <select name="store_locator_map[direction]" id="store_locator_direction">
                    <option <?php echo (isset($map_options['direction']) && $map_options['direction'] == 'detail') ?"selected" : ""; ?> value="detail"><?php esc_html_e("On map detailed direction","store_locator") ?></option>
                    <option <?php echo (isset($map_options['direction']) && $map_options['direction'] == 'gmap') ?"selected" : ""; ?> value="gmap"><?php esc_html_e("Redirect to Google maps","store_locator") ?></option>
                </select>
            </td></tr>
            <tr><td>
                    <label title="<?php _e("Will not apply if 'Fit Screen To Stores' is enabled.",'store_locator'); ?>" for="store_locator_map_zoom"><?php esc_html_e("Default Map Zoom Level:","store_locator"); ?></label>
                </td>
            ...
            <tr><td>
                <label title="<?php esc_html_e("Will not apply if 'Fit Screen To Stores' is enabled.",'store_locator'); ?>" for="store_locator_map_search_zoom"><?php esc_html_e("Location Search Zoom Level:","store_locator"); ?></label>
                </td>
                <td><input value="<?php echo isset($map_options['markerzoom']) ? esc_attr($map_options['markerzoom']) : 8; ?>" type="number" id="store_locator_map_search_zoom" name="store_locator_map[markerzoom]">
            </td></tr>
            <tr><td>
                <label title="<?php esc_html_e('Fit Screen To Stores','store_locator'); ?>" for="store_locator_fit_screen"><?php esc_html_e("Fit Screen To Stores:","store_locator"); ?></label>
            </td><td><input <?php echo (isset($map_options['fit_screen']) && ($map_options['fit_screen'])==1)?'checked':''; ?> value="1" type="checkbox" id="store_locator_fit_screen" name="store_locator_map[fit_screen]" >
                </td></tr>
            <tr><td>
                <label title="<?php esc_html_e("Will not apply if 'Fit Screen To Stores' is enabled.",'store_locator'); ?>" for="store_locator_fill_radius"><?php esc_html_e("Fill Radius with color:","store_locator"); ?></label>
                </td>
                <td><input <?php echo (isset($map_options['fill_radius']) && ($map_options['fill_radius'])==1)?'checked':''; ?> value="1" type="checkbox" id="store_locator_fill_radius" name="store_locator_map[fill_radius]" >
            </td></tr>
            <tr><td>
                <label title="<?php esc_html_e("Will only apply if 'Fill Radius with color' is enabled.",'store_locator'); ?>" for="store_locator_fill_radius_color"><?php esc_html_e("Fill color:","store_locator"); ?></label>
                </td>
                <td><input value="<?php echo isset($map_options['fill_color']) && !empty($map_options['fill_color'])? $map_options['fill_color'] :'#ccc'; ?>" type="text" id="store_locator_fill_radius_color" name="store_locator_map[fill_color]" class="wpmsl-color-field" placeholder="Enter Color Code" >
            </td></tr>

            <tr><th>
                <label title="<?php esc_html_e('Choose search options here. the default one will be between square brakets','store_locator'); ?>" for="store_locator_map_radius"><?php esc_html_e("Search radius options", 'store_locator'); ?>:</label>
            </th><td>
            ...
            <tr><th>
                <label title="<?php esc_html_e('Enable disable clusters on map','store_locator'); ?>" for="store_locator_map_cluster"><?php esc_html_e("Show Map Cluster", 'store_locator'); ?>?</label>
            </th><td>
                <input <?php echo (isset($map_options['mapcluster']) && ($map_options['mapcluster'])==1)?'checked':''; ?> value="1" type="checkbox" id="store_locator_map_cluster" name="store_locator_map[mapcluster]" >
           </td> </tr>
           <tr><th>
                <label title="<?php esc_html_e('On page load get location','store_locator'); ?>" for="store_locator_getlocation"><?php esc_html_e("On page load get location ", 'store_locator'); ?>?</label>
            </th><td>
                <input <?php echo (isset($map_options['getlocation']) && ($map_options['getlocation'])==1)?'checked':''; ?> value="1" type="checkbox" id="store_locator_getlocation" name="store_locator_map[getlocation]" >
           </td> </tr>
            <tr><th>
                <label title="<?php esc_html_e('Scroll Map to screen after search','store_locator'); ?>" for="store_locator_map_scroll"><?php esc_html_e("Scroll to map top after search", 'store_locator'); ?>?</label>
            </th><td>
            ...
            <tr><th>
                <label title="<?php esc_html_e('Enable the user to change the map type from the frontend','store_locator'); ?>" for="store_locator_map_mapTypeControl"><?php esc_html_e("Show map type control", 'store_locator'); ?>?</label>
            </th><td>
            ...
            <tr><th>
                <label title="<?php esc_html_e('Enable/Disable zoom by scroll on map','store_locator'); ?>" for="store_locator_map_scroll"><?php esc_html_e('Zoom by scroll on map', 'store_locator'); ?>?</label>
            </th><td>
            ...
            <tr><th>
                <label title="<?php esc_html_e('Check to hide Points of Interest in "Standard Map"','store_locator'); ?>" for="store_locator_map_mappoi"><?php esc_html_e('Hide POI (Points of Interest)', 'store_locator'); ?>?</label>
            </th><td>
                <input value="0" type="hidden" name="store_locator_map[mappoi]" >
                <input <?php echo (isset($map_options['mappoi']) && $map_options['mappoi'])?'checked':''; ?> value="1" type="checkbox" id="store_locator_map_mappoi" name="store_locator_map[mappoi]" >
            </td></tr>


            <tr><th>
                <label title="<?php esc_html_e('Display default Map Search','store_locator'); ?>" for="store_locator_default_search"><?php esc_html_e('Show Map Search options', 'store_locator'); ?></label>
            </th><td>
            ...
            <tr>
            	<th>
                <label title="<?php esc_html_e('Hide Field Options','store_locator'); ?>" for="store_locator_default_search"><?php esc_html_e('Hide Fields for Search', 'store_locator'); ?></label>
            </th>
            <td>
                <ul class="hide_fields">
                    ...
                    <li><input <?php echo (isset($map_options['search_field_radius']))?'checked':''; ?> value="hide-field" type="checkbox" id="search_field_get_my_location" name="store_locator_map[search_field_radius]" ><?php _e('Radius Field','store_locator');?></li>
                    ...
                    <li><input <?php echo (isset($map_options['reset_map']))?'checked':''; ?> value="hide-field" type="checkbox" id="search_field_reset_map" name="store_locator_map[reset_map]" ><?php esc_html_e('Reset Map','store_locator');?></li>
                </ul>
            </td>
            </tr>
            <tr><th>
                <label title="<?php esc_html_e('Switch To RTL','store_locator'); ?>" for="rtl_enabled"><?php esc_html_e("Switch To RTL", 'store_locator'); ?></label>
            </th><td>
            ...
            <p>
                <label title="Retro Map" for="store_locator_map_style2">
                    <img src="<?php echo esc_url(STORE_LOCATOR_PLUGIN_URL . 'assets/img/retro.png'); ?>" />
                    <?php esc_html_e("Retro Map", 'store_locator'); ?>
                    <input value="3" type="radio" id="store_locator_map_style2" name="store_locator_map[map_style]" <?php checked(3, $map_style); ?>>
                </label>
            </p>
            <p>
                <label title="Dark Map" for="store_locator_map_style2">
                    <img src="<?php echo esc_url(STORE_LOCATOR_PLUGIN_URL . 'assets/img/dark.png'); ?>" />
                    <?php esc_html_e("Dark Map", 'store_locator'); ?>
                    <input value="4" type="radio" id="store_locator_map_style2" name="store_locator_map[map_style]" <?php checked(4, $map_style); ?>>
                </label>
            </p>

            <p>
                <label title="Night Map" for="store_locator_map_style2">
                    <img src="<?php echo esc_url(STORE_LOCATOR_PLUGIN_URL . 'assets/img/night.png'); ?>" />
                    <?php esc_html_e("Night Map", 'store_locator'); ?>
                    <input value="5" type="radio" id="store_locator_map_style2" name="store_locator_map[map_style]" <?php checked(5, $map_style); ?>>
                </label>
            </p>
            <p>
                <label title="Aubergine Map" for="store_locator_map_style2">
                    <img src="<?php echo esc_url(STORE_LOCATOR_PLUGIN_URL . 'assets/img/aubergine.png'); ?>" />
                    <?php esc_html_e("Aubergine Map", 'store_locator'); ?>
                    <input value="6" type="radio" id="store_locator_map_style2" name="store_locator_map[map_style]" <?php checked(6, $map_style); ?>>
                </label>
            </p>
            <p>
                <label title="Basic Map" for="store_locator_map_style2">
                    <img src="<?php echo esc_url(STORE_LOCATOR_PLUGIN_URL . 'assets/img/basic.png'); ?>" />
                    <?php esc_html_e("Basic Map", 'store_locator'); ?>
                    <input value="7" type="radio" id="store_locator_map_style2" name="store_locator_map[map_style]" <?php checked(7, $map_style); ?>>
                </label>
            </p>
        </div>
        ...
        <hr>
            <p>
                <label title="Choose the color of user marker" for="store_locator_map_type"><strong><?php esc_html_e("User Marker", 'store_locator'); ?>:</strong></label>
              ...
              <?php 
               ...
               <div class="<?php echo $class; ?>">
                <img src="<?php echo $marker1; ?>" width="50px" height="50px">
                  <input type="hidden" value="<?php echo ($class=='wpmsl_custom_marker') ? esc_url($marker1) : ''; ?>" name="store_locator_map[marker1_custom]" />
                  <p><?php echo $uploadRemove; ?></p>
              </div>
            </p>
             <hr>
            <p>
                <label title="Choose the color of store marker" for="store_locator_map_type"><strong><?php esc_html_e("Store Marker", 'store_locator'); ?>:</strong></label>
            ...
            <p>
               ...
               <div class="<?php echo $class; ?>">
                <img src="<?php echo $marker2; ?>" width="50px" height="50px">
                  <input type="hidden" value="<?php echo ($class=='wpmsl_custom_marker') ? esc_url($marker2) : ''; ?>" name="store_locator_map[marker2_custom]" />
                  <p><?php echo $uploadRemove; ?></p>
              </div>
            </p>
            <p>
                <label title="<?php esc_html_e('Use category icons instead','store_locator'); ?>" for="store_locator_map_icons_cat"><?php esc_html_e("Use category markers for store markers", 'store_locator'); ?></label>
            ...
            ?>
             <hr>
            <p>
                <label title="<?php esc_html_e('You can customise the contetnt of the info window here by adding HTML if you need. Also you can use the below variables in the content','store_locator'); ?>" for="store_locator_map_infowindow"> <strong><?php esc_html_e("Info Window Content", 'store_locator'); ?>: </strong><p class="store_locator_tip">placeholders: {image} {address} {city} {state} {country} {zipcode} {name} {phone} {email} {website} {categories} {description} {working_hours}</p></label>
            ...
            <p>
                <label title="<?php esc_html_e('You can customize the look of the map by adding styles here','store_locator'); ?>" for="store_locator_map_style"><strong><?php esc_html_e("Customised Map Style", 'store_locator'); ?>:</strong> <p class="store_locator_tip"><?php _e('You can get some styles from','store_locator');?><a target="_blanck" href="https://snazzymaps.com"> <?php _e('Snazzy Maps','store_locator');?></a></p> </label>
            ...
            <p><?php $url=home_url().'/wp-multi-store-locator/'.$post->ID; ?>
            	<label for="embed_map_locator"><strong><?php esc_html_e('Embed Map','store_locator'); ?></strong></label><br>
            	<textarea class="widefat" rows="5" cols="70" id="embed_map_locator" name="embed_map_locator"><iframe style="height: 99vh; margin: 0; padding: 0px;" src="<?php echo esc_url($url); ?>" width="100%" frameborder="0" scrolling="no" allowfullscreen="allowfullscreen"></iframe>
            	</textarea>
            </p>
        </div>
        <?php
	         ...
	         <!-- Grid settings -->
                <table class="wpml-table">
                    ...
                    <tr><th>
                        ...
                        <label title="<?php esc_html_e('Maximum number of markers to be displayed:','store_locator'); ?>" for="store_locator_grid_total_markers"><?php esc_html_e('Maximum number of markers to be displayed:','store_locator'); ?></label>
                        </th><td>
                        <input type="text" class="regular-text" name="store_locator_grid[total_markers]" value="<?php echo !empty($grid_options['total_markers']) ? $grid_options['total_markers'] : ''; ?>" />
                    </td></tr>
	                <tr class="hide_if_fullscreen_map hide_if_regular_map"><th>
                        <label title="<?php esc_html_e('Map Result Show on','store_locator'); ?>" for="store_locator_map_type"><?php esc_html_e("Map Result Show on", 'store_locator'); ?>:</label>
                         </th><td>
        ...
        <div class="store_locator_map_settings" <?php echo (isset($placeholder_setting['global']) &&  $placeholder_setting['global']=='yes') ? 'style="display:none;"' : ''; ?>>
            ...
            -->
            <tr><th>
	        <?php esc_html_e('Store list text','store_locator');?>
	        	</th><td>
	        <input type="text" class="regular-text" name="placeholder_settings[store_list]" value="<?php echo !empty($placeholder_setting['store_list']) ? esc_attr($placeholder_setting['store_list']) : ''; ?>"/>
	        </td></tr>
            <tr><th>
	        <?php esc_html_e('Visit Website text','store_locator');?>
	        	</th><td>
	        <input type="text" class="regular-text" name="placeholder_settings[visit_website]" value="<?php echo !empty($placeholder_setting['visit_website']) ? esc_attr($placeholder_setting['visit_website']) : ''; ?>"/>
	        </td></tr>
		    </tbody>
		</table>
		</div>
